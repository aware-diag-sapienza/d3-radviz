...
function computeRepresentativeEntry(dimensions){
    ...
    dimensions.forEach(d => {
        representativeEntry.dimensions[d.id] = d3Array.sum(d.values);
    });
    dimensions.forEach(d => {
        representativeEntry.dimensions[d.id] /= d3Array.sum(Object.keys(representativeEntry.dimensions), d => representativeEntry.dimensions[d]);
    });
...
function computeDimensionsDominanceMean(entries){
    ...
    entries.forEach(e => {
        const values = dims.map(d => e.dimensions[d]);
        const vMax = d3Array.max(values);
        const discountedMean = (d3Array.sum(values) - vMax) / values.length;
        for(let i=0; i<values.length; i++){
...
function Radviz() {
    ...
    let dragged = function(d) {
        console.log('evento',d3Selection.event, 'd',d);
